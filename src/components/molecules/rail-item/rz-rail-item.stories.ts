...
const meta: Meta = {
  ...
  tags: ['autodocs'],
  args: { index: 2, label: '23 · Todi', active: false, done: false },
  render: ({ index, label, active, done }) =>
    html`<div style="width:220px">
      <rz-rail-item index=${index} label=${label} ?active=${active} ?done=${done}></rz-rail-item>
    </div>`,
};
export default meta;
type Story = StoryObj;
...
export const Rail: Story = {
  render: () => html`
    <div style="width:220px;display:flex;flex-direction:column;gap:2px">
      ...
      <rz-rail-item index="3" label="7 · Bilawal"></rz-rail-item>
      <rz-rail-item index="4" label="41 · Todi"></rz-rail-item>
    </div>
  `,
};
